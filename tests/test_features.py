from src.features import compute_rolling_engagement
import pandas as pd
...
def test_compute_rolling_engagement():
    ...
    for _ in range(4):
        u2_events.append(
            {"user_id": "u2", "days_since_signup": 0, "trial_length_days": 3}
        )
    for _ in range(3):
        u2_events.append(
            {"user_id": "u2", "days_since_signup": 1, "trial_length_days": 3}
        )
    for _ in range(2):
        u2_events.append(
            {"user_id": "u2", "days_since_signup": 2, "trial_length_days": 3}
        )
    for _ in range(1):
        u2_events.append(
            {"user_id": "u2", "days_since_signup": 3, "trial_length_days": 3}
        )
