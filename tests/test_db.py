...
def test_validate_headline_finding_sql(mock_db):
    ...
    pd.DataFrame(users_data).to_sql(
        "users_clean", conn, if_exists="replace", index=False
    )
    ...
    pd.DataFrame(feature_usage_clean_data).to_sql(
        "feature_usage_clean", conn, if_exists="replace", index=False
    )
    ...
    pd.DataFrame(user_features_data).to_sql(
        "user_features", conn, if_exists="replace", index=False
    )
