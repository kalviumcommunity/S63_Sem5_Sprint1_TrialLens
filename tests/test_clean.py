...
def test_run_cleaning(dirty_db):
    ...
    cursor.execute(
        "SELECT plan_type, company_size FROM users_clean WHERE user_id = 'u1'"
    )
    ...
    cursor.execute(
        "SELECT is_outlier_total_event_count FROM users_clean WHERE user_id = 'u99'"
    )
    outlier_row = cursor.fetchone()
    assert outlier_row[0] == 1  # True (SQLite boolean is 1/0)

    cursor.execute(
        "SELECT is_outlier_total_event_count FROM users_clean WHERE user_id = 'u10'"
    )
