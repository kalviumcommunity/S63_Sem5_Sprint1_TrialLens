...
def run_cleaning(db_path="data/trialens.db"):
    ...
    df_users, users_norm = normalize_strings(
        df_users, ["plan_type", "company_size"], case="title"
    )
    report["users_normalized"] = int(users_norm)
    print(f"Normalized {users_norm} values across plan_type and company_size in users.")

    df_features, features_norm = normalize_strings(
        df_features, ["feature_name"], case="lower"
    )
    ...
    print(
        f"Checked users for null user_id/signup_date: Dropped {dropped_users_nulls} rows."
    )
    ...
    print(
        f"Checked feature_usage for null user_id/event_timestamp: Dropped {dropped_features_nulls} rows."
    )
    ...
    print(
        f"Checked feature_usage for duplicate event_id: Dropped {dropped_feature_dupes} rows."
    )
    ...
    print(
        f"Checked users for logical inconsistencies (converted vs conversion_date): Dropped {dropped_invalid_users} rows."
    )

    # 5. Out of window feature_usage
    initial_features = len(df_features)
    df_users["signup_date_dt"] = pd.to_datetime(
        df_users["signup_date"], errors="coerce"
    )
    df_features["event_timestamp_dt"] = pd.to_datetime(
        df_features["event_timestamp"], errors="coerce"
    )

    # Left merge to preserve features for the orphaned check later
    merged = df_features.merge(
        df_users[["user_id", "signup_date_dt", "trial_length_days"]],
        on="user_id",
        how="left",
    )

    trial_lengths = pd.to_numeric(merged["trial_length_days"], errors="coerce").fillna(
        14
    )
    ...
    print(
        f"Checked feature_usage for out-of-window events: Dropped {dropped_out_of_window} rows."
    )
    ...
    # 7. Outlier Detection
    event_counts = (
        df_features.groupby("user_id").size().reset_index(name="total_event_count")
    )
    ...
    print(
        f"Data cleaning complete. Final users: {len(df_users)}, Final feature_usage: {len(df_features)}."
    )
