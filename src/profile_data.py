...
def run_profiling(db_path="data/trialens.db"):
    ...
    try:
        ...
        for table in tables:
            ...
            print(f"\n=== Table: {table} ===")
            print(
                f"Rows: {table_report['row_count']}, Columns: {table_report['column_count']}"
            )
            ...
            for col in df.columns:
                ...
                print(f"  - Column: {col}")
                print(
                    f"    dtype: {dtype} | nulls: {null_pct:.1f}% | unique: {unique_vals}"
                )
                ...
                if pd.api.types.is_numeric_dtype(series):
                    ...
                    print(
                        f"    [Numeric] min: {c_min}, max: {c_max}, mean: {c_mean:.2f}"
                        if c_mean is not None
                        else "    [Numeric] All NaN"
                    )
                else:
