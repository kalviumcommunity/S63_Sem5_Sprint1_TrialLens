...
def compute_daily_activity(feature_usage_df, user_id):
    ...
    trial_length = (
        user_events["trial_length_days"].iloc[0]
        if "trial_length_days" in user_events.columns
        else 14
    )
...
def build_features(db_path="data/trialens.db"):
    ...
    df = features.merge(
        users[["user_id", "signup_date_dt", "trial_length_days"]],
        on="user_id",
        how="left",
    )
    ...
    distinct_features_used = (
        g["feature_name"].nunique().rename("distinct_features_used")
    )

    # core_features_used_first_7_days
    core_first_7 = (
        df[df["is_core"] & df["is_first_7_days"]]
        .groupby("user_id")["feature_name"]
        .nunique()
        .rename("core_features_used_first_7_days")
    )
    ...
    first_core_days = (
        core_only.groupby("user_id")["days_since_signup"]
        .min()
        .rename("time_to_first_core_feature")
    )
    ...
    user_features["rolling_trend_slope"] = user_features["rolling_trend_slope"].fillna(
        0.0
    )
    ...
    final_df = users[["user_id", "converted", "plan_type", "company_size"]].merge(
