...
def run_ingestion(
    db_path="data/trialens.db",
    users_csv="data/raw/users.csv",
    feature_usage_csv="data/raw/feature_usage.csv",
):
    """
    ...
    cursor.execute(
        "CREATE INDEX idx_feature_usage_event_timestamp ON feature_usage(event_timestamp)"
    )
    ...
    if not os.path.exists(users_csv) or not os.path.exists(feature_usage_csv):
        print(
            f"Error: CSV files not found. Ensure {users_csv} and {feature_usage_csv} exist."
        )
        conn.close()
        return
    ...
    print(
        f"Orphaned feature_usage rows (referential integrity check): {orphaned_count}"
    )

    if orphaned_count > 0:
        print(
            "WARNING: Found orphaned rows in feature_usage! Referential integrity violated."
        )
    else:
