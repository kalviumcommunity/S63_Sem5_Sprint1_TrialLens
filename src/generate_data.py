...
def generate_synthetic_data(output_dir="data/raw"):
    ...
    for _ in range(NUM_USERS):
        ...
        plan_type = np.random.choice(
            ["Starter", "Pro", "Enterprise"], p=[0.5, 0.3, 0.2]
        )
        ...
        is_high_engagement = (
            np.random.rand() < 0.35
        )  # ~35% of users are "high engagement"
        ...
        if converted:
            ...
            conversion_date = signup_date + timedelta(
                days=conv_day, hours=np.random.randint(0, 24)
            )
        ...
        for day, num_events in enumerate(events_per_day):
            ...
            for e in range(num_events):
                event_timestamp = session_start + timedelta(
                    minutes=e * np.random.randint(2, 15)
                )
                user_events.append(
                    {
        ...
        first_7_events = [
            e for e in user_events if (e["event_timestamp"] - signup_date).days < 7
        ]
        other_events = [
            e for e in user_events if (e["event_timestamp"] - signup_date).days >= 7
        ]
    ...
    df_merged = pd.merge(
        df_features, df_users[["user_id", "signup_date", "converted"]], on="user_id"
    )
    df_merged["days_since_signup"] = (
        df_merged["event_timestamp"] - df_merged["signup_date"]
    ).dt.days
    ...
    user_core_counts = (
        core_only.groupby("user_id")["feature_name"]
        .nunique()
        .reset_index(name="distinct_core_first_7")
    )
    ...
    print(
        f"Conversion Rate (High Engagement - 3+ core features in first 7 days): {conv_high:.1%}"
    )
    print(
        f"Conversion Rate (Low Engagement - <3 core features in first 7 days): {conv_low:.1%}"
    )
