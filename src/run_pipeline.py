...
def main(test_data_dir=None):
    parser = argparse.ArgumentParser(
        description="Run the full TrialLens data pipeline."
    )
    parser.add_argument(
        "--force-regenerate",
        action="store_true",
        help="Regenerate synthetic data even if raw CSVs exist.",
    )
    ...
    if args.force_regenerate or not (
        os.path.exists(users_csv) and os.path.exists(features_csv)
    ):
        run_step(
            1, "Generating Synthetic Data", generate_synthetic_data, output_dir=raw_dir
        )
    else:
        print("\n=== Step 1/7: Generating Synthetic Data ===")
        print(
            "Raw CSVs already exist. Skipping generation (use --force-regenerate to override)."
        )

    # Step 2: Ingestion
    run_step(
        2,
        "Ingesting Data to SQLite",
        run_ingestion,
        users_csv=users_csv,
