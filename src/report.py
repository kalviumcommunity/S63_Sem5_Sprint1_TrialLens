...
def generate_report_text(
    analysis_report: Dict[str, Any], kpi_summary: Dict[str, Any]
) -> str:
    """
    ...
    lines.append(
        f"- **Overall Conversion Rate**: {kpi_summary.get('overall_conversion_rate', 0.0):.1f}%"
    )
    lines.append(
        f"- **Avg Time to Convert**: {kpi_summary.get('avg_time_to_convert', 0.0):.1f} days\n"
    )
    ...
    sig_text = (
        ...
        if hc.get("significant")
        else "not statistically significant"
    )

    lines.append(
        f"Users who engage with 3 or more core features within their first 7 days convert at "
    ...
    for seg_col, segment_data in segments.items():
        ...
        for val, metrics in segment_data.items():
            ...
            n = metrics["count"]
            lines.append(
                f"- **{val}**: >=3 Core: {high:.1f}% | <3 Core: {low:.1f}% (N={n})"
            )
        lines.append("")
